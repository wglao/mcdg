...
parser.add_argument("--alpha3", default=20, type=int)
parser.add_argument("--alpha4", default=1., type=float)
...
filename = 'Wave_eq_DG_GNN_NO_EMBED_receiver_flux_dim' + str(int(args.alpha3)) +'MCalpha_' + str(mc_alpha) + '_noise_' + str(noise_level) + '_lr_' + str(learning_rate) + '_batch_' + str(batch_size) + '_nseq_' + str(n_seq) + '_num_epochs_' + str(num_epochs)

if wandb_upload:
    import wandb
    wandb.init(project="DG_GNN_mcTangent_Approach", entity="hainguyenpho", name = filename)
    wandb.config.problem = 'Wave/transport equation'
...
Train_data = pd.read_csv('../data/Train_noise_' + str(noise_level) +'_d_'  + str(num_train) + '_Nt_' + str(nt_step_train) + '_K_' + str(K) + '_Np_' + str(N) + '.csv') 
...
def net_embed(graphs_tuple):
    embedder = GraphMapFeatures(embed_node_fn = hk.Sequential([hk.Linear(int(args.alpha3)), jax.nn.relu, hk.Linear(int(args.alpha3))]))
...
def run_epoch(opt_state, data):
    opt_state, _ = lax.fori_loop(0, num_batches, body_fun, (opt_state, data))
    return opt_state

def TrainModel(train_data, test_data, num_epochs, opt_state, wandb_upload):
    ...
    for epoch in range(num_epochs):
        ...
        opt_state = run_epoch(opt_state, train_data)
        ...
        if (not wandb_upload) & (epoch % 1 == 0):  # Print MSE every 100 epochs
            print("Data_d {:d} n_seq {:d} batch {:d} lr {:.2e} loss {:.2e} TE {:.2e}  TE_min {:.2e} EPmin {:d} EP {} Time 1 {:.3e}s".format(
                num_train, n_seq, batch_size, learning_rate, train_loss, test_accuracy, test_accuracy_min, epoch_min, epoch, Epoch_training_1))
...
#! STEP6:: Training
best_opt_state, end_opt_state = TrainModel(Train_data, Test_data, num_epochs, opt_state, wandb_upload)
