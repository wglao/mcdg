...
Solution_samples_array.to_csv('data/Train_noise_' + str(0.00) +'_d_'  + str(num_train) + '_Nt_' + str(nt_step_train) + '_K_' + str(K) + '_Np_' + str(N) + '.csv', index=False)
...
Solution_samples_array.to_csv('data/Test_d_'  + str(num_test) + '_Nt_' + str(nt_step_test) + '_K_' + str(K) + '_Np_' + str(N) + '.csv', index=False)
...
print(test_data.shape)
Solution_samples_array = pd.DataFrame({'samples': test_data.flatten()})
Solution_samples_array.to_csv('data/Test_d_'  + str(num_test) + '_Nt_' + str(nt_test) + '_K_' + str(K) + '_Np_' + str(N) + '.csv', index=False)
...
def element_adding_noise(element_nodal_values, nosie_vec):
    return element_nodal_values + noise_level * nosie_vec * jnp.max(element_nodal_values)
...
batch_timestep_adding_noise = jit(jax.vmap(timestep_adding_noise, in_axes=(0, 0)))
...
train_noise.to_csv('data/Train_noise_' + str(noise_level) +'_d_'  + str(num_train) + '_Nt_' + str(nt_step_train) + '_K_' + str(K) + '_Np_' + str(N) + '.csv', index=False)
...
Solution_samples_array = pd.DataFrame({'samples': np.asarray([len(senders_data)])})
